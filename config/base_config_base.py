...
# ==================== URL Check 模式配置 ====================
# 工作模式：validity（仅检测有效性）| metrics（仅抓指标）| both（同时检测+抓指标）
URLCHECK_MODE = "both"
# 每批从外部库读取的 URL 数量
URLCHECK_BATCH_SIZE = 10
# 是否在 url_check 模式下同时抓取评论
URLCHECK_ENABLE_COMMENTS = False
# url_check 模式下单个作品最大评论抓取数
URLCHECK_MAX_COMMENTS = 15

# ==================== AI 字段映射配置 ====================
# 火山引擎 Doubao API（OpenAI 兼容格式）
# 实际值通过 .env 中 DOUBAO_API_KEY / DOUBAO_BASE_URL 注入
